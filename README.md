...
관리자 화면의 “오늘 상담 요약 테스트”는 `testTodayCounselingSummary()`를 통해 자동 트리거와 같은 `sendCounselingSummary()` 발송 로직을 사용합니다. 수동 테스트는 설정값과 중복 발송 상태를 우회하지만, 자동 발송은 `DISCORD_DAILY_SUMMARY_ENABLED=true`와 당일 중복 발송 방지 상태를 함께 확인합니다. 자동 알림이 오지 않으면 관리자 설정의 “운영 상태 점검”에서 실제 트리거 핸들러와 시간대를 먼저 확인하세요.
...
- 예약 카드의 **예약 변경**: 관리자 세션 인증 후 기존 예약 행의 날짜·차시만 안전하게 변경합니다. 학생 비밀번호와 메모는 유지하며, 완료된 상담은 변경할 수 없습니다.
- 학생별 상담 이력
- 학사일정과 상담 불가 기간 CRUD
- 날짜별 상담 가능 시간
